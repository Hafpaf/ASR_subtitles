...
This project uses the Automatic Speech Recognition (ASR) model [OpenAI Whisper](https://github.com/openai/whisper) to create subtitles for talks and and other video's. 
...
An example of wrong word recognition with the base model, is the word 'batch' can be recognized as 'patch' in some cases, but can be correctly recognized with the medium model. Read the [OpenAI Whisper model card](https://github.com/openai/whisper/blob/main/model-card.md) and the paper [Robust Speech Recognition via Large-Scale Weak Supervision by Radford et. al](https://arxiv.org/abs/2212.04356) for more information on transcription precision.
...
|  Processor | Model | Time |
| --- | --- | --- |
|3 GHz CPU | base model | 15 min 12 sec |
| Nvidia Tesla M60 | base model | 1 min 36 sec |
| Nvidia Tesla M60 | medium model | 7 min 11 sec |
...
# Thanks to
- [OpenAI Whisper](https://github.com/openai/whisper) for their wonderful model
